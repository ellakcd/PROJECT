function showProfile(results) {
	...
	if (results["active"] === true) {
		$("#looking_or_not").html("LOOKING FOR APT: True<br><button id='deactivate'> Not Currently Looking For Apt? </button>");
		$("#deactivate").on("click", deactivateUser);
	} else {
		$("#looking_or_not").html("LOOKING FOR APT: False<br><button id='activate'> Looking Once More! </button>");
